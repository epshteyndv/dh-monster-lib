...
- Каталог **`data/monsters/`**: любое количество файлов с расширением **`.yaml`**. В каждом файле — корневой ключ **`monsters`**: массив карточек в стиле **Daggerheart** (имя, тир, флейвор, мотивы, блок статов, опыт, фичи). При сборке все файлы объединяются в один каталог (порядок файлов — по имени; `id` не должны повторяться между файлами).
